...
namespace Gauss {
	template <class Real_t> class EMEstimator {
		...
		const size_t EM_ITERS = 4000;
		const Real_t MIN_COMPONENT_VARIANCE = 0.000000001;

		std::vector<std::vector<Real_t>> get_component_membership_weights() {
			std::vector<std::vector<Real_t>> result = Matrix::create_2d_matrix(means.size(), data.size(), 0.0);
			std::vector<Real_t> densities_sums = Matrix::create_1d_matrix(data.size(), 0.0);

			for (size_t component = 0; component < means.size(); component++) {
				for (size_t arg = 0; arg < data.size(); arg++) {
					const Real_t density = gaussian_density(data[arg], means[component], variances[component]) * weights[component];
		...
		void re_estimate(std::vector<std::vector<Real_t>> &probabilities) {
			for (size_t component = 0; component < means.size(); component ++) {
				const Real_t cluster_probs = std::accumulate(probabilities[component].begin(), probabilities[component].end(), 0.0);
				Real_t variance_estimator = 0.0;
				for (size_t arg = 0; arg < data.size(); arg++) {
					variance_estimator += probabilities[component][arg] * (data[arg] - means[component]) * (data[arg] - means[component]);
				}

				variances[component] = variance_estimator / cluster_probs;
				if (variances[component] < MIN_COMPONENT_VARIANCE || std::isnan(variances[component])) {
	...
	public:
		EMEstimator<Real_t>(std::vector<Real_t> data, Random<Real_t> &random): data{data}, random {random} {}
		...
		LikelihoodData<Real_t> estimate(const size_t mixture_size) {
			...
			std::transform(variances.begin(), variances.end(), variances.begin(), [](Real_t v) -> Real_t { return std::sqrt(v); });
