...
void sample_tree_test() {
  ...
  for (auto n : nodes) {
    if (n != tree.get_root()) {
      IS_TRUE(std::find(events.begin(), events.end(), tree.get_node_event(n)) !=
              events.end())
    }
  }
  END_TEST;
...
void prunning_test() {
  ...
  std::vector<std::pair<size_t, size_t>> attachment{
      std::make_pair(87, 119),  std::make_pair(121, 137),
      std::make_pair(110, 119), std::make_pair(179, 217),
      std::make_pair(45, 65),   std::make_pair(58, 63),
      std::make_pair(26, 28),   std::make_pair(47, 52),
      std::make_pair(148, 164), std::make_pair(105, 106)};
