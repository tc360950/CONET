 ...
 * @param delimiter
 * @return std::vector<std::vector<std::string>> Each row corresponds to one
 * line, columns are lines split by the delimiter
 */

std::vector<std::vector<std::string>> split_file_by_delimiter(std::string path,
                                                              char delimiter) {
...
string_matrix_to_int(std::vector<std::vector<std::string>> data) {
...
string_matrix_to_double(std::vector<std::vector<std::string>> data) {
  std::vector<std::vector<double>> double_data(data.size());
  for (size_t i = 0; i < double_data.size(); i++) {
    double_data[i].resize(data[0].size());
    std::transform(data[i].begin(), data[i].end(), double_data[i].begin(),
                   [](std::string s) -> double { return std::stold(s); });
  };
  return double_data;
}
 ...
 *
 * Create vector of positive numbers, where each element from the vector @i is
 * such that
...
CONETInputData<double>
diff_matrix_to_cell_provider(std::vector<std::vector<double>> data) {
  const size_t CHROMOSOME_ROW = 0;
  const size_t BETWEEN_BINS_LENGTH_ROW = 1;
  const size_t DIFFS_START_ROW = 2;

  CONETInputData<double> provider(data[0].size(),
                                  get_chromosome_markers(data[CHROMOSOME_ROW]),
                                  data[BETWEEN_BINS_LENGTH_ROW]);
  for (size_t i = DIFFS_START_ROW; i < data.size(); i++) {
    std::for_each(data[i].begin(), data[i].end(),
                  [](double &r) { r = -std::abs(r); });
    provider.post_cell(data[i]);
  }
  return provider;
}

void read_counts_penalty_files(CONETInputData<double> &provider,
                               std::string summed_counts_path,
                               std::string squared_counts_path,
                               char delimiter) {
  auto summed_counts = string_matrix_to_double(
      split_file_by_delimiter(summed_counts_path, delimiter));
  auto squared_counts = string_matrix_to_double(
      split_file_by_delimiter(squared_counts_path, delimiter));
  auto regions_sizes = summed_counts[0];
  summed_counts.erase(summed_counts.begin());
  squared_counts.erase(squared_counts.begin());
  provider.post_counts_dispersion_data(regions_sizes, summed_counts,
                                       squared_counts);
}
...
CONETInputData<double> create_from_file(std::string path,
                                        ...
                                        std::string squared_counts_path,
                                        char delimiter) {
  CONETInputData<double> provider = diff_matrix_to_cell_provider(
      string_matrix_to_double(split_file_by_delimiter(path, delimiter)));
  read_counts_penalty_files(provider, summed_counts_path, squared_counts_path,
                            delimiter);
  return provider;
}
