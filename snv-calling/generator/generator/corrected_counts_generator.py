...
import numpy as np
from generator.generator.gen_utils import sample_conditionally


@dataclass
class CNSampler:
    """
    This class provides copy number sampling functionalities for the generative model.
    """

    CN_to_weight: Dict[
        int, float
    ]  # CNs will be sampled using weights provided by the user
    CN_to_sd: Dict[
        int, float
    ]  # Each CN is assigned a sd - CC for this CN will be sampled by adding gaussian noise
    # with this deviation
    neutral_cn: int
    noise_prob: float
    ...
    def sample_non_neutral_CN(self) -> int:
        return sample_conditionally(
            sampler=lambda: np.random.choice(
                list(self.CN_to_weight.keys()), 1, p=list(self.CN_to_weight.values())
            )[0],
    ...
    def __sample_truncated_normal(mean: float, sd: float) -> float:
        return sample_conditionally(
            sampler=lambda: np.random.normal(mean, sd, 1), condition=lambda x: x > 0
        )
