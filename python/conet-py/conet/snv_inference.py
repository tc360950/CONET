...
class NewtonRhapsonEstimator:
    ...
    def solve(self, params: Parameters) -> Parameters:
        ...
        while np.max(np.abs(target)) >= 0.1 and i < 10000:
            target = self.calculate_target(params)
            print(f"Target after {i} iterations {target} with {params}")
    ...
    def calculate_jacobian(self, p: Parameters) -> np.ndarray:
        ...
        f_1_e = (
            0.0
            if np.sum(S_0 * S_0 * B_inv_2) == 0.0
            else -math.exp(math.log(np.sum(S_0 * S_0 * B_inv_2)) + 2.0 * log_coef)
        )
        ...
        if np.sum(S_0) > 0:
            f_1_q = (
                ...
                * math.exp(math.log(np.sum(S_0)) - 2.0 * math.log(p.q))
                ...
                + +0.0
                if np.sum(S_0 * A * B_inv_2) == 0.0
                else math.exp(
                    math.log(np.sum(S_0 * A * B_inv_2)) - 2.0 * math.log(p.q) + log_coef
                )
                     + -0.0
                if np.sum(S_0 * B_inv) == 0.0
                else math.exp(math.log(np.sum(S_0 * B_inv)) - 2.0 * math.log(p.q))
            )

        f_2_q = (
                -math.log(1 - p.q) * math.exp(math.log(np.sum(SCN)) - 2.0 * math.log(p.q))
...
class NewtonRhapsonEstimatorOnlyQ:
    MAX_Q: float = 0.999
    ...
    def solve(self, params: Parameters) -> Parameters:
        ...
        i = 0
        target = self.calculate_target(params)
        while np.max(np.abs(target[2])) >= 0.1 and i < 10000:
            target = self.calculate_target(params)
            print(f"Target after {i} iterations {target} with {params}")
            jacobian = self.calculate_jacobian(params)
            diff_vec = target[2] / jacobian

            if p_vec[2] - diff_vec < 0:
                diff_vec = 0.7 * p_vec[2]
            p_vec[2] = p_vec[2] - diff_vec
            params.e = p_vec[0]
            params.m = p_vec[1]
            params.q = p_vec[2]
            params.q = min(params.q, NewtonRhapsonEstimator.MAX_Q)
